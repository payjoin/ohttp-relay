...
/// A helper type representing an absolute base URL where an OHTTP gateway might
/// be found.

#[derive(PartialEq, Eq, Clone, Copy, Debug)]
pub struct Policy {
    pub bip77_allowed: bool,
    pub expires: Instant,
}
...
impl Prober {
    ...
    pub async fn assert_opt_in(&self, url: &GatewayUri) -> Option<()> {
    ...
    pub async fn check_opt_in(&self, url: &GatewayUri) -> Option<Policy> {
        let inflight = {
            let mut locked_map = self.gateways.write().await;
    ...
    pub async fn unavailable_for(&self) -> Duration {
        let mut locked_map = self.gateways.write().await;
...
impl Default for TTLConfig {
    fn default() -> Self {
        ...
        Self {
            // Long lived cache for opt-in with no Cache-Control
            opt_in: LONG,

            // Long lived caching for servers that are not OHTTP gateways or
            // that don't care about opt-in
            http_2xx: LONG,
            http_4xx: LONG,

            // Short live caching for struggling servers
            http_5xx: SHORT,

            // No caching transient errors that should be retried
            http_504_gateway_timeout: NONE,
            dns: NONE, // DNS resolver should cache
            reset_by_peer: NONE,
            timedout: NONE,

            // For other errors, enforce rudimentary rate limiting
            default: SHORT,
        }
    }
}
...
mod tests {
    ...
    async fn test_known_gateways() {
        ...
        if let Some(Status::Known(got)) = db.get(&url) {
            ...
            assert!(false, "initially inserted policy should be retrivable");
        }
        ...
        if let Some(Status::InFlight(got)) = db.get(&url) {
            ...
            assert!(false, "inflight entry should be retrievable");
        }
    ...
    async fn test_mock_opt_in() {
        ...
        let mock_opt_in = server
            ...
            .with_header(hyper::header::CONTENT_TYPE, ALLOWED_PURPOSES_CONTENT_TYPE)
        ...
        mock_opt_in.assert();
        mock_opt_in.remove_async().await;
    ...
    async fn test_mock_no_opt_in() {
        ...
        let mock_only_rfc_9540 = server
            .mock("GET", RFC_9540_GATEWAY_PATH)
            .match_query(mockito::Matcher::Regex("^allowed_purposes$".into()))
            .with_header(hyper::header::CONTENT_TYPE, "application/ohttp-keys")
        ...
        mock_only_rfc_9540.remove_async().await;
    }
    ...
    async fn test_mock_404() {
        ...
        mock_not_found.remove_async().await;
    }
    ...
    async fn test_inflight_deduplication() {
        ...
        mock_delayed.remove_async().await;
    }
    ...
    fn test_parse_alpn_encoded() {
        ...
        assert_eq!(result.unwrap().len(), 0, "a list with 0 elements should be valid");
        ...
        assert_eq!(
            ...
            1,
            "a list with 1 element of 0 length should be valid"
        );
        assert_eq!(
            result.as_ref().unwrap()[0].len(),
            0,
            "the single element should be of length 0"
        );
        ...
        assert_eq!(
            result.as_ref().unwrap().len(),
            1,
            "list with single 1 byte element should be valid"
        );
        assert_eq!(
            result.as_ref().unwrap()[0].len(),
            1,
            "the length of the element should be 1 byte"
        );
        assert_eq!(result.as_ref().unwrap()[0][0], b'a', "the element value should be parsed");

        let result = parse_alpn_encoded(b"\x00\x02\x01\x00\x00");
        assert_eq!(result.as_ref().unwrap().len(), 2, "two element list should be valid");
        assert_eq!(
            result.as_ref().unwrap()[0].len(),
            1,
            "the first element should be a 1 byte long"
        );
        assert_eq!(result.as_ref().unwrap()[0][0], 0, "the first element should be a null byte");
        assert_eq!(result.as_ref().unwrap()[1].len(), 0, "the second element should be empty");

        let result = parse_alpn_encoded(BIP77_OPT_IN_RESPONSE);
        assert_eq!(
            result.as_ref().unwrap().len(),
            1,
            "pre canned BIP 77 opt-in response should have 1 element"
        );
        assert_eq!(
